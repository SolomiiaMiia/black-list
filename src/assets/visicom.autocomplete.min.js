let visicomAutoComplete = (function () {
  function visicomAutoCompleteFunction(options) {
    ...
    addEvent(input, 'input', function () {
      ...
      timer = setTimeout(function () {
        ...
        if (searchText.length > 0) {
          ...
          results = [];
          makeGeocodeRequest(searchText).then(function (json) {
    ...
    addEvent(input, 'focus', function (e) {
      if ((suggests.length > 0 && !suggestsDiv.classList.contains('open')) || suggestsDiv.contains(notFoundElement)) {
        suggestsDiv.classList.add('open');
      }
      e.stopPropagation();
    });
    ...
    addEvent(getElement('body'), 'click', function (e) {
      if (e.target !== suggestsDiv && e.target !== input && e.target !== closeDiv && (!suggestsDiv.hasChildNodes() || (suggestsDiv.hasChildNodes() && [].slice.call(suggestsDiv.childNodes).every(function (el) {
          return e.target !== el;
        })))) {
        suggestsDiv.classList.remove('open');
      }
    ...
    function makeGeocodeRequest(text) {
      ...
      let url = opt.proxyApiGeocodeUrl ? opt.proxyApiGeocodeUrl + '?text=' + opt.searchTextPrefix + text + '&lang=' + opt.lang + ci_param + ce_param + intersect + contains + '&key=' + opt.apiKey + '&limit=' + opt.suggestsLimit : opt.apiUrl.replace('{lang}', opt.lang) + '/geocode.json?text=' + opt.searchTextPrefix + text + ci_param + ce_param + intersect + contains + '&key=' + opt.apiKey + '&limit=' + opt.suggestsLimit + '&country=' + opt.country;
    ...
    function makeFeatureRequest(feature_id) {
      let url = opt.proxyApiFeatureUrl ? opt.proxyApiFeatureUrl + '?feature_id=' + feature_id + '&lang=' + opt.lang + '&key=' + opt.apiKey : opt.apiUrl.replace('{lang}', opt.lang) + '/feature/' + feature_id + '.json?key=' + opt.apiKey;
      return fetch(encodeURI(url)).then(function (data) {
        return data.json();
      });
    }
    ...
    function renderOnMap(suggest) {
      ...
      if (suggest.feature.coords !== undefined) {
        ...
        makeFeatureRequest(suggest.feature.id).then(function (data) {
    ...
    function trimToLength(d, qu, isCompact) {
      return isCompact ? (d.length >= opt.maxCharsInSuggest ? replaceWithBold(d.substring(0, opt.maxCharsInSuggest).concat('...'), qu) : replaceWithBold(d, qu)) : d;
    }

    function replaceWithBold(dat, que) {
      let description = dat.toLowerCase();
      let quer = que.toLowerCase();
      return description.indexOf(quer) < 0 ? dat : dat.substring(0, description.indexOf(quer)) + '<strong>' + dat.substring(description.indexOf(quer), description.indexOf(quer) + que.length) + '</strong>' + dat.substring(description.indexOf(quer) + que.length);
    }
    ...
    Coordparsing.format = function (latlng) {
      function _split(position) {
        ...
        result.secondsDecimal = Math.min(9, Math.floor((position - result.seconds) * 10));
        return result;
      }
      let format = "DDD°MM′SS″H";
      let lat = _split(latlng[0]);
      let lng = _split(latlng[1]);
      lat = format.replace('H', lat.hemisphere === 1 ? 'N' : 'S').replace('DDD', lat.degrees).replace('MM', lat.minutes).replace('SS', lat.seconds);
      lng = format.replace('H', lng.hemisphere === 1 ? 'E' : 'W').replace('DDD', lng.degrees).replace('MM', lng.minutes).replace('SS', lng.seconds);
      return '' + lat + lng;
    };
    Coordparsing.parse = function (text) {
      ...
      function detectLatLng(txt, digits) {
        ...
        if (l.length === 1) {
          ...
          else if (l[0] === 'S') return {
            'lat': -digits[0],
            'lng': digits[1]
          };
          else if (l[0] === 'W') return {
            'lng': -digits[0],
            'lat': digits[1]
          };
        } else if (l.length === 2) {
